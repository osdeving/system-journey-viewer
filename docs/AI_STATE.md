...
- UI appearance:
  ...
  - `Preferences > Toolbar sections` exposes `Show main menu` and `Place toolbar beside app icon`; the same toggles are available from Settings/command palette when the menu is visible/searchable,
  - docked windows use a compact header band for tabs/actions and arrow-based vertical scroll controls instead of visible dock-host scrollbar chrome,
