...
- UI appearance:
  ...
  - `Preferences > Toolbar sections` exposes `Show main menu` and `Place toolbar beside app icon`; the same toggles are available from Settings/command palette when the menu is visible/searchable, and the app logo restores the hidden main menu in compact mode,
