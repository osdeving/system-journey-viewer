...
- Original plan status after the latest delivery:
  ...
  - Phase 4 (`Guided Tutorial engine`): in progress. Slice 1 delivered the guided overlay tutorial, slice 2 added action-gated steps, and slice 3 fixes the spotlight so highlighted UI stays visible (segmented backdrop cutout) while expanding action-gated interactions (toolbar mode click and canvas click). Remaining work still includes richer target types, deeper task-oriented steps, and more advanced action-wait predicates.
