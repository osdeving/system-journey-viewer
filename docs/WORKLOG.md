...
- Fixed Kafka/queue cylinder rendering so users can stretch the node horizontally without enlarging the right-side elliptical face.
- Removed the previous bounds-clamping approach so database and Kafka/queue nodes can keep arbitrary user-controlled widths.

### Changes

- `apps/web/src/diagram/nodes/nodeShapePaths.ts`
  - changed horizontal queue/Kafka cylinder geometry so `capRx` is derived from node height instead of node width; the body can grow while the visible end face stays stable.
- `apps/web/src/components/canvas/DiagramCanvas.tsx`, `apps/web/src/store/useEditorStore.ts`, `apps/web/src/layout/autoArrange.ts`
  - removed cylinder width constraints from resize, store updates, snapshot hydration, and auto-arrange sizing.
- `apps/web/src/diagram/nodes/nodeShapePaths.test.ts`, `apps/web/src/store/useEditorStore.test.ts`
  - added/updated regression coverage for fixed queue cap geometry, preserved wide resize bounds, preserved wide persisted bounds, and auto-arrange widening.
...
- `npm --workspace @sjv/web run test:run`
- `npm --workspace @sjv/web run build`
- Playwright/Firefox visual check against `http://127.0.0.1:5173/`: forced a wide Kafka/DB snapshot and confirmed wide bounds remain while the queue/Kafka cap path keeps a stable `capRx`.
