...
- Original plan status after the latest delivery:
  ...
  - Phase 4 (`Guided Tutorial engine`): in progress. Slice 1 delivered the guided overlay tutorial and slice 2 adds action-gated steps (menu-open and click/open requirements) with non-blocking overlay interaction so highlighted controls remain clickable. Remaining work still includes richer target types, action-gated steps that wait for more complex interactions, and a deeper task-oriented walkthrough.
