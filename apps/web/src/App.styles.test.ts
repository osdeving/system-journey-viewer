...
describe('App boundary styles', () => {
  ...
  it('styles drawer tabs, dock panel and monaco dsl layout', () => {
    ...
    expect(appCss).toContain('.journey-step-item-selected')
    expect(appCss).toContain('.journey-step-context-menu')
    expect(appCss).toContain('.journey-step-icon-action')
  ...
  it('contains visual styles for node shapes and color palette', () => {
    ...
    expect(appCss).toContain('.node-tech-icon-glyph')
    expect(appCss).toContain('.node-tech-icon-edit-frame')
    expect(appCss).toContain('.node-tech-icon-resize')
  ...
  it('includes desktop menu and advanced canvas interaction styles', () => {
    ...
    expect(appCss).toContain('.command-palette-list-shell')
    expect(appCss).toMatch(/\.command-palette-list\s*\{[^}]*scrollbar-width:\s*none;[^}]*-ms-overflow-style:\s*none;/s)
    expect(appCss).toContain('.command-palette-list::-webkit-scrollbar')
    expect(appCss).toContain('.command-palette-list-shell.command-palette-results-scroll-bottom::after')
    expect(appCss).toContain('.command-palette-row-active')
    expect(appCss).not.toMatch(/\.command-palette-row-active\s*\{[^}]*linear-gradient/s)
    expect(appCss).not.toMatch(/\.theme-dark\s+\.command-palette-row-active\s*\{[^}]*linear-gradient/s)
    expect(appCss).toMatch(/\.command-palette-section\s*\{[^}]*font-weight:\s*400;/s)
    expect(appCss).toMatch(/\.command-palette-copy strong\s*\{[^}]*font-weight:\s*400;/s)
    expect(appCss).toMatch(/\.command-palette-meta kbd\s*\{[^}]*font-weight:\s*400;/s)
    expect(appCss).toContain('.confirmation-dialog-backdrop')
    expect(appCss).toContain('.confirmation-dialog')
    expect(appCss).toContain('.confirmation-dialog-danger')
    expect(appCss).toContain('.confirmation-dialog-button-primary')
    ...
    expect(appCss).toContain('.freeform-shape-preview')
    expect(appCss).toContain('.topbar-dock-placement-strip')
    ...
    expect(appCss).toMatch(/\.palette-search\s*\{[^}]*width:\s*100%;[^}]*background:\s*transparent;[^}]*\}/s)
    expect(appCss).toMatch(/\.palette-category-strip\s*\{[^}]*display:\s*grid;[^}]*width:\s*100%;[^}]*\}/s)
    expect(appCss).toMatch(/\.managed-host-sidebar\s+\.dock-panel\s*\{[^}]*border-radius:\s*0;[^}]*\}/s)
